...
OPTIONS:
  ...
  -I IDENTIFIER     Snapshot identifier default -lvmsnapsht
...
while getopts ':m:e:g:c:i:I:ndqh' OPTION ; do
    case $OPTION in
        m)
...
if [ $1 = "create" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        cleanup
	outputne "Remove snapshot devmapper links ...  $SNAPSHOTPATH ..."
	$DMSETUP remove $SNAPSHOTPATH > /dev/null 2>&1
	if [ $? -eq 0 ]; then
            status_success "ok"
        else
            status_error "failed"
            error "Error while removing snapshot using dmsetup, please remove manually. $DMSETUP remove $SNAPSHOTPATH ... "
        fi
	...
	#done
 	...
 	outputne "Remove -cow devmapper links.  $MAPPERPATH/${GROUPNAME}-${SNAPSHOTNAME//-/--}-cow ..."
	$DMSETUP remove ${GROUPNAME}-${SNAPSHOTNAME//-/--}-cow > /dev/null 2>&1
   
        if [ $? -eq 0 ]; then
            status_success "ok"
        else
            status_error "failed"
            error "Error while removing snapshot using dmsetup, please remove manually.  $DMSETUP remove ${GROUPNAME}-${SNAPSHOTNAME//-/--}-cow  "
        fi
else
        status_error "failed"
